...
def risk_adjusted_table(risk_adjusted):
    """
    Publication-ready table of actual vs. risk-adjusted predicted hit rates by race.
    Shows actual hit rate, predicted hit rate, and residual gap with 95% CIs.
    """

    df = risk_adjusted.copy()

    # Compute standard errors from n_searches for actual hit rate
    # predicted hit rate is a model mean so we report it without CI
    df["actual_se"] = np.sqrt(
        df["actual_hit_rate"] * (1 - df["actual_hit_rate"]) / df["n_searches"]
    )

    # Format columns
    df["Actual Hit Rate (%)"] = df.apply(
        lambda row: fmt_est_ci(row["actual_hit_rate"] * 100, row["actual_se"] * 100, digits=2),
        axis=1
    )
    df["Predicted Hit Rate (%)"] = df["predicted_hit_rate"].apply(
        lambda x: f"{x * 100:.2f}"
    )
    df["Residual (pp)"] = df["residual"].apply(
        lambda x: f"{x * 100:+.2f}"
    )
    df["Searches"] = df["n_searches"].astype(int)

    # Enforce race ordering
    df.index = pd.CategoricalIndex(df.index, categories=TABLE_RACE_ORDER, ordered=True)
    df = df.sort_index()
    ...
    result.index.name = "Canonical Race"

    return result


def kitchen_sink_table(race_results):
    """
    Publication-ready table of kitchen sink logistic regression results.
    Shows log-odds coefficients and odds ratios with 95% CIs and p-values.
    Reference category is White.
    """
    df = race_results.copy()

    # Format log-odds with CI
    df["Log-Odds (95% CI)"] = df.apply(
        lambda r: f"{r['Coef.']:.3f} ({r['[0.025']:.3f}, {r['0.975]']:.3f})",
        axis=1
    )

    # Compute and format odds ratios with CI
    df["Odds Ratio (95% CI)"] = df.apply(
        lambda r: f"{np.exp(r['Coef.']):.3f} ({np.exp(r['[0.025']):.3f}, {np.exp(r['0.975]']):.3f})",
        axis=1
    )

    # Format p-value
    df["P-value"] = df["P>|z|"].apply(
        lambda p: "<0.001" if p < 0.001 else f"{p:.3f}"
    )

    result = df[["Log-Odds (95% CI)", "Odds Ratio (95% CI)", "P-value"]].copy()
    result.index.name = "Race (reference: White)"

    # Enforce race ordering
    race_order_ks = ["Black/African American", "Hispanic/Latino", "Asian", "Other"]
    result = result.reindex(race_order_ks)
